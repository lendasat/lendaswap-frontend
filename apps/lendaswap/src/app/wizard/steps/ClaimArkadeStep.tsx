...
export function ClaimArkadeStep({ swapData }: ClaimArkadeStepProps) {
  ...
  // Auto-claim VHTLC when server is funded
  useEffect(() => {
    const autoClaimArkade = async () => {
      ...
      try {
        ...
        console.log("Auto-claiming VHTLC:", {
          swapId: swapData.id,
          retryCount,
        });
              ...
              {swapData.status === "serverfunded" && (
                <div className="from-primary/5 to-card mt-2 space-y-2 rounded-lg border bg-gradient-to-t p-4">
                  <p className="text-sm font-medium">
                    {isClaiming ? "Redeeming your sats..." : "VHTLC Funded"}
                  </p>
